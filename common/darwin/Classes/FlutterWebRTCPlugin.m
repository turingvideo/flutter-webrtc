...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  if ([@"initialize" isEqualToString:call.method]) {
    ...
    result(nil);
  } else if ([@"videoRendererUpdateBaseTileOffset" isEqualToString:call.method]) {
    NSDictionary* argsMap = call.arguments;
    NSNumber* textureId = argsMap[@"textureId"];
    FlutterRTCVideoRenderer* render = self.renders[textureId];
    if (!render) {
      result([FlutterError errorWithCode:@"videoRendererUpdateBaseTileOffset: render is nil"
                                 message:nil
                                 details:nil]);
      return;
    }
    float panDeg = [argsMap[@"panDeg"] floatValue];
    float tiltDeg = [argsMap[@"tiltDeg"] floatValue];
    [render updateBaseTileOffset:panDeg tiltDeg:tiltDeg];
    result(nil);
  }
#if TARGET_OS_IPHONE || TARGET_OS_OSX
